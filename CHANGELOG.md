...
- Declarative lockstep version check: the extension passes `initializationOptions: { expectedAwsumVersion, preferButtonsOverLinks: true }` and the server warns on mismatch via `window/showMessageRequest` with an action button that opens the install page through `window/showDocument`. The expected version comes from the IDE-side plugin descriptor (the same value that `gradle.properties:pluginVersion` injects into `plugin.xml` at build time via `patchPluginXml`).
